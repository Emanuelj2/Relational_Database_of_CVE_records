...
def fetch_average_cvss_scores(tree):
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor()

        # SQL query to compute average CVSS scores grouped by product name
        query = """
             SELECT 
            products.NAME AS Product_Name, 
            AVG(cve_records2.CVSS_SCORE) AS Average_CVSS_Score
        FROM 
            products
        JOIN 
            cve_records2 
        ON 
            products.ID = cve_records2.ID
        GROUP BY 
            products.NAME
        """
        print(f"Executing query: {query}")  # Debugging line
        ...
        # Update treeview columns
        tree["columns"] = ("Product_Name", "Average_CVSS_Score")
        tree.heading("Product_Name", text="Product Name")
        tree.heading("Average_CVSS_Score", text="Average CVSS Score")
        tree.column("Product_Name", anchor="w")
        tree.column("Average_CVSS_Score", anchor="center")
...
def display_piechart():
    try:
        # Call the function from analyze_data.py to generate the combined piechart
...
def open_gui():
    ...
    # Button to display average CVSS scores
    avg_button = ttk.Button(input_frame, text="Display Average CVSS Scores",
                            command=lambda: fetch_average_cvss_scores(tree))
    avg_button.grid(row=6, column=0, pady=10)
